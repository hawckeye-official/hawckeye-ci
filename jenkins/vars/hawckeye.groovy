...
def call(Map cfg = [:]) {
  ...
  String waitForUrl = (cfg.waitForUrl ?: 'false').toString()
  String expectedRef = cfg.expectedRef ?: (env.GIT_COMMIT ?: '')
  ...
  withCredentials([string(credentialsId: hawkeyeCred, variable: 'HAWKEYE_TOKEN')]) {
    withEnv([
      ...
      "HAWKEYE_WAIT_FOR_URL=${waitForUrl}",
      "HAWKEYE_EXPECTED_REF=${expectedRef}",
      "HAWKEYE_VERSION_PATH=${versionPath}",
